...
export function Toaster() {
    ...
    return (
        <ToastProvider> {/* Provider to manage the toast context */}
            {toasts.map(function ({ id, title, description, action, ...props }) {
                // Map over each toast to render it
                return (
                    <Toast key={id} {...props}> {/* Render the Toast component with unique key */}
                        <div className="grid gap-1"> {/* Container for title and description */}
                            {title && <ToastTitle>{title}</ToastTitle>} {/* Render title if it exists */}
                            {description && <ToastDescription>{description}</ToastDescription>} {/* Render description if it exists */}
                        </div>
